...
| `iter-04-patch-04-arrows-setautoshapes.md` | Elemzési nyilak továbbra sem jelentek meg — `cg.set({ drawable: { autoShapes } })` az `anim()` pathon fut, SVG render skip optimalizáció miatt nem rajzolódott újra | `syncCg()` két hívásra bontva: `cg.set({ brushes })` + `cg.setAutoShapes(shapes)` — a dedikált API a `render()` pathon fut, garantált SVG újrarajzolás |
| `iter-04-patch-05-arrows-no-fen-sync.md` | Elemzési nyilak még mindig nem jelennek meg — `syncCg()` fen-nel hívta `cg.set()`-t az analysisLines watcherből is, az `anim()` path és a chessground RAF debounce race condition okozta a megbízhatatlan SVG renderelést | `syncArrows()` elkülönítve: `cg.set({ drawable: { brushes } })` fen nélkül → `render()` path (nem `anim()`) → nincs race, user annotációk megmaradnak |
